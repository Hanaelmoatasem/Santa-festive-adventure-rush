...
float gingerbreadManRotationAngle = 0.0f;

void updateRotation() {
	gingerbreadManRotationAngle += 1.0f; // Increment the angle
	if (gingerbreadManRotationAngle > 360.0f) {
		gingerbreadManRotationAngle -= 360.0f; // Keep the angle in the range [0, 360]
	}
...
void myDisplaylevel2(void){
	...
	if (player.isAlive()&& level2==true) {
		...
		if (!ginger[0].collected) {
			glPushMatrix();
			glTranslated(-14, 0, -5);
			glRotatef(gingerbreadManRotationAngle, 0.0f, 1.0f, 0.0f);
			glRotatef(90.f, 1, 0, 0);
